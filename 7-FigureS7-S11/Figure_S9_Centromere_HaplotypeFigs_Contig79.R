#### ####  CONTIG79 AKA X CHROMOSOME CENTROMERE
...
library(gdsfmt)
library(SNPRelate)
snpgdsVCF2GDS("All_filtered_snps.variant.select.Contig79.nosat.vcf","Contig79.nosat.gds",method ="biallelic.only")

genofile1<-snpgdsOpen("Contig79.nosat.gds")
set.seed(100)
...
vcf <- read.vcfR("All_filtered_snps.variant.select.Contig79.2.vcf", verbose = FALSE)
dna <- ape::read.dna("Contig79.fasta", format = "fasta")
gff <- read.table("Contig79.gff", sep="\t", quote="")


# Create a chromR object.
chrom <- create.chromR(name="Contig79", vcf=vcf, seq=dna, ann=gff, verbose=TRUE)
...
gt.proc3 <- gt.proc %>% mutate(pos = as.numeric(pos), sample = as.character(sample)) %>% 
  arrange(sample,pos) %>% filter(pos >= 5655, pos <=48432)
...
gt.te.x <- gt.te %>% filter(contig=="Contig79")
...
te.combos.j1 <- gt.te.x %>% filter(genotype == "Jockey_3_Dmel_08212020") %>% tidyr::expand(contig_pos, sample) #, geno.recode)
te.combos.j2 <- te.combos.j1 %>% left_join(gt.te.x %>% select(c(contig_pos, sample, genotype, geno.recode ))) 
...
te.combos.j2 <- te.combos.j2 %>% filter(geno.recode != "Other")
...
#pdf("TE_poly_allTE_Contig119_ordered_1.pdf",width=10,height=8)
te.combos.j3 %>% ggplot() +
  geom_raster(aes(x = contig_pos, y = sample, fill = geno.recode)) +
...
pdf("Contig79_SNPraster_p1.pdf",width=8,height=6)
p1 <- gt.proc4 %>% ggplot() +
  geom_raster(aes(x = contig_pos, y = sample, fill = geno.recode)) +
  scale_y_discrete(expand = expansion(add=c(1, 0))) +
  #geom_rect(aes(ymin=-2, xmin=1, ymax=-1, xmax=1000), fill="steelblue") +
...
pdf("Contig79_J3raster_p2.pdf",width=8,height=6)
...
pdf("Contig79_SNPs_J3_tanglegrame_p3.pdf",width=14,height=6)
dendextend::dendlist(
  den %>% 
    dendextend::set("branches_lty", 1), #%>%
  denJ3 %>% 
